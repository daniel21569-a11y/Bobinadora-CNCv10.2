...
inline void init_honeycomb_cycle() {
  motor_state_c1.pasos_X_recorrido_ida =
      (long)(Sistema::config_nido_abeja.ancho_carrete *
             Hardware::Motor::PASOS_POR_MM_X);
  ...
  // Pre-calcular ratio UNA SOLA VEZ
  motor_state_c1.step_ratio_X_per_Y_honeycomb =
      (float)motor_state_c1.pasos_X_recorrido_ida /
      (float)motor_state_c1.mitad_ciclo_Y;
...
inline void process_honeycomb_step() {
  ...
  // Determinar fase IDA o VUELTA
  bool en_ida =
      (motor_state_c1.pasos_Y_en_ciclo_actual < motor_state_c1.mitad_ciclo_Y);

  // Incrementar acumulador con ratio pre-calculado
  motor_state_c1.X_step_accumulator_honeycomb +=
      motor_state_c1.step_ratio_X_per_Y_honeycomb;

  // ✅ GENERAR PASOS CON DIRECCIÓN ESTABLECIDA EN CADA PASO (COMO
  // TRANSFORMADOR)
  while (motor_state_c1.X_step_accumulator_honeycomb >= 1.0f) {
    set_dir_x_fast(en_ida); // ✅ Dirección en CADA paso
    pulse_step_pin(Hardware::Motor::STEP_X_PIN);
    ...
    motor_state_c1.X_step_accumulator_honeycomb -= 1.0f;
  }

  // Actualizar tracking
  motor_state_c1.direccion_X_honeycomb = en_ida;
  ...
  if (motor_state_c1.pasos_Y_en_ciclo_actual >=
    ...
    // Corrección final si hay deriva
    if (abs(Sistema::estado.pasos_X_acumulados) > 5) {
      long pasos_correccion = abs(Sistema::estado.pasos_X_acumulados);
      bool direccion_correccion =
          (Sistema::estado.pasos_X_acumulados > 0) ? false : true;
